...
function ArticlesTable() {
  ...
  const { articles, history } = useSelector((state) => state.api);
  const {
    startOffset, endOffset, itemsPerPage, currentPage,
  ...
  useEffect(() => {
    const onScroll = () => {
      if (window.scrollY > 0) setIsScrollbar(true);
          ...
          articles.slice(startOffset, endOffset).map((art) => (
            <tr key={art.id}>
              <td>
                <Nav.Link
                  as={Link}
                  to={`/details/${art.id.replaceAll('/', '_')}`}
                >
                  {art.title}
                </Nav.Link>
              </td>
